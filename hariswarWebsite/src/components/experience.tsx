...
export const Experience = () => {
  ...
  const clearHoverIfStale = () => {
    ...
    const containerHovered = !!containerRef.current?.matches(':hover');
  ...
  // Intersection Observer for sonar ping on entry
  useEffect(() => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      if (containerRef.current) {
        ...
        // If the user scrolls while a card was previously hovered, React may not fire
        // a mouseleave event. Ensure bullets only shoot when the card is truly hovered.
        clearHoverIfStale();
      }
    };
  ...
  // Extra safety: if hover gets "stuck" (e.g., during fast scroll), clear it on pointer movement
  useEffect(() => {
                ...
                <div className={isLeft ? '' : 'order-2'}>
                  <div
                    ...
                    onMouseEnter={() => {
                      ...
                      // Flip submarine based on card position
                      const isLeft = idx % 2 === 0;
                      setSubmarineTilt({
                        rotate: isLeft ? 0 : 180,
